...
from attack_angle import plot_tracking_angles, create_tracking_score_df, generate_track_angle_plot
...
def get_slider_values():
    ...
    """
    return {
        'contact_locations': [st.session_state.get(f'contact_location_{i}_default', 0.5) for i in range(4)],
        'hunting_dists': [st.session_state.get(f'hunting_{i}_default', 1.0) for i in range(4)],
        'track_angles': [st.session_state.get(f'track_angle_{i}_default', 5) for i in range(4)],
        'metric_order': [st.session_state.get(f'priority_{i}_default', 'Contact Location') for i in range(3)]
    }
...
def display_scorecard():
    ...
    column_name_map = {'batter': 'Batter ID',
...
# Tabs for UI
tab_selection = st.sidebar.radio("Select Tab",
...
if tab_selection == "Customize Grading":
    ...
    with col1:
        contact_location = [st.slider(widget_grades[i], -1.0, 2.0,
                                      ...
                                      step=0.25,
                                      key=f"contact_location_{i}") for i in range(4)]
    ...
    with col1:
        hunting = [st.slider(widget_grades[i], 0.0, 3.0,
                             st.session_state[f'hunting_{i}_default'],
                             step=0.25,
                             key=f"hunting_{i}") for i in range(4)]
    ...
    with col1:
        track_angle = [st.selectbox(widget_grades[i], [2.5, 5, 10, 15],
                                    [2.5, 5, 10, 15].index(st.session_state[f'track_angle_{i}_default']),
                                    key=f"track_angle_{i}") for i in range(4)]
...
#------------------------------------------------
    ...
    metric_priorities = [st.sidebar.selectbox(f"Priority {i+1}",
                                              metric_options,
    ...
    swing_count = st.sidebar.slider("Minimum Swing Count",
                                    min_value=2, max_value=25, value=st.session_state['swing_count_default'],
                                    key='swing_count',
                                    on_change=save_widget_states
                                    )
...
#------------------------------------------------
    ...
    if not scorecard.empty:
        ...
        batter_id = st.sidebar.selectbox('Select Batter', batter_list, key='batter_selector')
        ...
        def update_plots(batter_id):
            ...
            if batter_id:
                ...
                try:
                    hunt_grade = scorecard.query(f'batter == {batter_id}')['hunting_grade'].item()
                ...
                try:
                    loc_grade = scorecard.query(f'batter == {batter_id}')['timing_grade'].item()
                    batter_timing = timing_metrics_df.query(f'batter == {batter_id}')
                    loc_fig = viz_contact_loc(batter_timing, loc_grade, get_slider_values()['contact_locations'])
                    st.pyplot(loc_fig)
                except Exception as e:
                    st.error(f"Error in viz_contact_loc: {e}")
